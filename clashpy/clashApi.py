...
class ClashApi:
    ...
    def __init__(self, clanManager, email, password, key_name, clanTag, logger):
        ...
        self._logger = logger
        self._token = self._getToken(email, password, key_name)
    ...
    def _getToken(self, email, password, key_name):
        key_count = 1
        keys = []
        _keys = []
        KEY_MAXIMUM = 10
        ...
        if resp.status_code == 403:
            raise RuntimeError("Invalid developer credentials.")
        ...
        if "keys" in resp.json():
            keys = resp.json()["keys"]
            _keys.extend(key["key"] for key in keys if key["name"] == key_name and ip in key["cidrRanges"])

            self._logger.debug("Retrieved %s valid keys from the developer site.", len(_keys))

            if len(_keys) < key_count:
                for key in (k for k in keys if k["name"] == key_name and ip not in k["cidrRanges"]):
                    self._logger.debug(
                        "Deleting key with the name %s and IP %s (not matching our current IP address).",
                        key_name, key["cidrRanges"],
                    )
        ...
        if len(_keys) == 0:
            raise RuntimeError(
                "There are {} API keys already created and none match a key_name of '{}'.".format(len(keys), key_name)
            )
    ...
    def getApiData(self):
        ...
        return {
            "updateMember": self._updateMember,
